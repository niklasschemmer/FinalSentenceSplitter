...
using SentenceSplitterTest.Neural_Net.Neuron.Connection;

namespace SentenceSplitterTest.Neural_Net
{
    [Serializable]
    class NeuralNet
    {
        ...
        public void TrainNetwork(List<int> input, List<float> result)
        {
            for (var i = 0; i < 4; i++)
            {
                for (var j = 0; j < OutputLayer.Count; j++)
                {
                    OutputLayer[j].Error = Neuron.Neuron.SigmoidDerivate(OutputLayer[j].GetValue()) * (result[j] - OutputLayer[j].GetValue());
                ...
                foreach (var hidden in HiddenLayer)
                {
                    foreach (var output in OutputLayer)
                    {
                        hidden.Error = Neuron.Neuron.SigmoidDerivate(hidden.GetValue()) * output.Error * output.Connections[output.Connections.FindIndex(p => p.entrieNeuron == hidden)].weight;
                    }
